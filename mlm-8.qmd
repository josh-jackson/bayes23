...
$${Y}_{ij} = b_{0} + b_{1}X_{ij} + ... +\epsilon_{ij}$$
...
Ignores any dependency. Treats people as exchangeable. ANOVA equivalent. 
...
Everyone is unqiue and we cannot learn from how others perform. 
```{r}
#| code-fold: true
...
The population we are sampling from can have a distribution (mu, sigma), and that distribution is different from the observed distribution of mu (or sigma)
...
$$\text{y}_{ij}  \sim \operatorname{Normal}(\mu_i, \sigma)$$
$$\mu_i  = \beta_{0[j]}$$ 
This line indicates we have varying intercepts that vary over individual j. Each parameter estimated needs a prior. 
$$\beta_{0[j]} \sim \operatorname{Normal}({\bar \mu},  \sigma_0)$$
$${\bar \mu}  \sim {\operatorname{Normal}(0, 1.5)}$$
$$\sigma_0 \sim {\operatorname{Exponential}(1)}$$
$$\sigma \sim {\operatorname{Exponential}(1)}$$
...
Our regularizing prior for correlation matrices. Has one parameter to tune the potential associations. 
