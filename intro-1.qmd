...
-   Bayes gives you basically the same results. We will talk about the algo later, but right now just think of it as different but the same. 

::: incremental
-   So why use it? It is a better swiss army knife. Always works. Fewer assumptions than ML. Some problems NEED it. Extra bonus benefits such as regularization, easier to simulate - plus many more. 
...
- In sum: we tend not to make guesses on the average parameter estimate, but instead constraints on the range of possible values. Constraints tell us how "surpised" we'd be if we get some estimate 
