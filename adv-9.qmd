...
Each missing value is imputed N times leading to a total of N fully imputed data sets. The imputation for each dataset is similar to our predict function, using variables to make a prediction. Sometimes this step is an iterative process. 
...
# assumes the probability that a value is missing depends only on observed values and not on unobserved values
```
...
MI is sort of like that. You could instear run 1 model, find the posterior, use that as a prior and then move on with the rest of the datasets repeating this process. Sort of like this MI process. 
...
CTT defines this as: true value = observed value + error
...
schoool.success (observed) \~ Normal( True.SS_i , SE.SS_i )
True.SS_i \~ Normal( $\mu_i$ , $\sigma$ )

$\mu_i$ = $\beta_0$ + $\beta_1$ $SS\_c$ 
...
From a binary setting to one in which we have multiple, ordered response categories such as with likert items. 
