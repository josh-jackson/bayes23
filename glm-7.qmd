...
or 
$$P.Death_i  \sim \operatorname{Bernoulli} (p_i)$$  
...
avg_slopes(glm.6, type = "response")
```
...
$$\operatorname{Pr} (y = 0 | p, \lambda)  = \operatorname{Pr} (\text{drink} | p) + \operatorname{Pr} (\text{work} | p) \times \operatorname{Pr} (0 | \lambda)$$
$$= p + (1 - p) \exp (- \lambda)$$
...
```{r}
#| code-fold: true
conditional_effects(glm.13, categorical =TRUE)
```
...
```{r}
plot_predictions(glm.13, condition = "action") +
    facet_wrap(~ group)
```
...
```{r}
plot_predictions(glm.13, condition = c("action", "contact"))
```
...
library(modelr)
trolley %>% 
  data_grid(action = seq_range(action, n = 200), response = c(1:7), contact = 1, intention = 1) %>% 
   add_epred_draws(glm.13) %>% 
  ggplot(aes(x = action, y = .epred, fill = factor(response))) +
  stat_lineribbon(.width = 0.95) + facet_wrap(~response)
