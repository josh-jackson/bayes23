...
-   If you have 20 groups, that is a lot of priors (one for each dummy!), but this approach there is a single prior for all levels/factors
...
-   One the one hand, multiple comparisons are about error rates, which reference NHST, and we don't have a null distribution in bayes land

-   On the other hand, the idea that chance results come up with more shakes of the dice is about probability. We still want to make good decisions and strengthen inferences.
...
-   Why not incorporate our model to expect outliers, especially when working with low N situations?

-   Enter the t-distribution, with itsheavy tailed distributions. Alternatively, a "robust" estimation could use medians instead of means
